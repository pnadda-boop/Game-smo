...
## ต้องคุยกับใครให้จบก่อน — **ปล่อยว่าง (ค่าเริ่มต้น) = ใช้งานได้ตั้งแต่เริ่มฉาก**
##
## ชี้ที่ตัวโหนด NPC ไม่ใช่ชื่อบท เพราะกล่องสนแค่ว่า "คุยจบหรือยัง"
## ไม่ได้สนว่าบทนั้นชื่ออะไร — เปลี่ยนบทของ NPC แล้วไม่ต้องกลับมาแก้ตรงนี้
##
## 🚨 **ค่าเริ่มต้นเคยเป็น `^"../Wawa"` — เอาออกแล้ว 2026-08-21**
## วาวาที่ห้องใต้ตึกถูกปิดไป (เหลือวาวาที่ชั้น 2 คนเดียว) แต่**กล่องต้องอยู่ต่อ**
## ค้างค่าเดิมไว้แล้วจะเป็นได้ 2 แบบ ซึ่งพังทั้งคู่:
##   ลบโหนดวาวาทิ้ง  → `get_node_or_null()` ได้ null → `push_error` แล้ว**ล็อกค้างตลอดฉาก**
##   แค่ซ่อนวาวาไว้   → หาโหนดเจอ แต่ `can_interact()` คืน false เพราะมองไม่เห็น
##                      → ไม่มีวันมีสัญญาณ `interacted` → **ล็อกค้างเหมือนกัน**
## ทั้งสองทางแปลว่าหยิบกล่องไม่ได้ = สายเนื้อเรื่องทั้งเส้นตัน
##
## ⚠️ **ค่าเริ่มต้นไม่ควรผูกกับตัวละครตัวใดตัวหนึ่งตั้งแต่แรก** — นี่เป็นสคริปต์กลาง
## ของ "ของที่หยิบได้" ทุกชิ้น การฝังชื่อ `Wawa` ไว้ในค่าเริ่มต้นทำให้กล่องใบใหม่
## ทุกใบเกิดมาพร้อมเงื่อนไขที่ไม่มีใครตั้งใจ · อยากได้เงื่อนไขก็กรอกที่ instance นั้น
@export var unlock_after_npc: NodePath = ^""
@export_group("")
