...
## ==============================================
## ช่วงที่ NPC ตัวนี้ "มีเควสค้างอยู่"
## ==============================================
## เงื่อนไขชุดเดียวคุมสองอย่างพร้อมกัน: **ไอคอนเควสบนหัว** และ **บทที่พูด**
## เข้าเควสเมื่อไหร่ก็ขึ้นไอคอนพร้อมเปลี่ยนบท · พ้นเควสก็หายพร้อมกลับบทเดิม
## แยกกันไม่ได้เพราะมันคือสถานะเดียวกันของตัวละคร
##
## ตัวอย่างจริง (โชแชง): `quest_if = "box_delivered"` · `quest_unless = "has_bread"`
## · `quest_dialogue_id = "Cho_break"`
## → วางกล่องเสร็จ = โชมีเบรคให้มารับ · รับไปแล้ว = กลับเป็นโชคนเดิม
##
## 🚨 **อยู่ใน `.tres` ไม่ใช่ที่โหนดในแมพ** — เป็นบทบาทของตัวละครในเนื้อเรื่อง
## ซึ่งเหมือนกันทุกฉากที่เขาโผล่ (กฎเดียวกับ `dialogue_id`)
## ปล่อยว่างทั้งสามช่อง = NPC ตัวนั้นไม่มีระบบเควส ใช้กฎเดิมทุกอย่าง

## ชื่อธง**บูล**ใน `GameState` ที่ต้องเป็นจริง (ว่าง = ไม่เช็ค)
@export var quest_if: String = ""
## ชื่อธง**บูล**ใน `GameState` ที่ต้องเป็นเท็จ (ว่าง = ไม่เช็ค) — ตัวปิดเควส
@export var quest_unless: String = ""
## คีย์บทที่ใช้ระหว่างอยู่ในเควส (ว่าง = ใช้บทปกติ เปลี่ยนแค่ไอคอน)
