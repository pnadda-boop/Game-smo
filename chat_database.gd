...
const HISTORY: Dictionary = {
	"smo": [
		...
		{"sender": "kaem", "text": "เดี๋ยวหนูกับเพื่อนทักไปค่ะ"},
		{"sender": "namking", "text": "อย่าลืมกินข้าวเช้ากันด้วยนะ"},
		{"sender": "namking", "text": "เดี๋ยวเป็นลมตอนทำกิจกรรม"},
		{"time": "06:30 น."},
		{"sender": "jaja", "text": "ใครมาถึงแล้วแยกไปหาหัวหน้าฝ่ายตัวเองได้เลยนะ"},
...
## ==============================================
## บทแชทที่ผูกกับ "เหตุการณ์ในเกม" ไม่ใช่กับการเปิดห้อง
## ==============================================
## 🚨 **ทำไมไม่ใช้ `INCOMING` ที่มีอยู่แล้ว**
## `INCOMING[group]` ถูกเล่น**ทุกครั้งที่เปิดห้องนั้น** — เอาข้อความของโชไปใส่ที่นั่น
## ฉาก `wokeup` จะเล่นมันตั้งแต่ตอนเช้า ทั้งที่ผู้เล่นยังไม่ได้วางกล่องด้วยซ้ำ
##
## เส้นแบ่งของสามตัวคือ **"อะไรเป็นตัวสั่งให้ข้อความมาถึง"**
##
## | ตัว | มาถึงเมื่อ |
## |---|---|
## | `HISTORY` | มาถึงก่อนผู้เล่นเปิดโทรศัพท์ — ขึ้นครบทันทีตอนเปิดห้อง |
## | `INCOMING` | มาถึงตอนผู้เล่นเปิดห้องนั้นดูอยู่ |
## | `EVENTS` | มาถึงตอน**เหตุการณ์ในฉากเกิดขึ้น** (วางกล่องเสร็จ ฯลฯ) |
##
## ⚠️ **`group` ชี้กลุ่มเดิมได้** — ข้อความของโชอยู่ในกลุ่ม SMO กลุ่มเดียวกับตอนเช้า
## เปิดขึ้นมาจะเห็นประวัติเช้าอยู่ครบแล้วข้อความใหม่ต่อท้าย เหมือนแอปแชทจริง
## (ไม่ต้องสร้างกลุ่มใหม่ กลุ่มคือ "ห้องคุย" ไม่ใช่ "ฉากของเนื้อเรื่อง")
const EVENTS: Dictionary = {
	## หลังผู้เล่นเอากล่องไปวางที่ห้องรับรองชั้น 2 เสร็จ
	"box_delivered": {
		"group": "smo",
		"lines": [
			{"time": "07:10 น."},
			{"sender": "chochang", "text": "ใครวางของเสร็จแล้ว มาเอาเบรคเช้าไปก่อนเลยนะครับ"},
			{"sender": "chochang", "text": "อยู่หน้าห้องประชุม"},
		],
	},
}


## ดึงบทของเหตุการณ์ทั้งก้อน คืน {} ถ้าไม่พบ
##
## ⚠️ `push_error` ตอนหาไม่เจอ ไม่ใช่คืนค่าว่างเงียบ ๆ — พิมพ์คีย์ผิดแล้วเงียบ
## = แชทเด้งขึ้นมาแล้วไม่มีข้อความอะไรเข้ามาเลย ซึ่งดูเหมือนระบบแชทพัง
## ไม่ใช่เหมือน "พิมพ์ชื่อเหตุการณ์ผิด"
func get_event(event_id: String) -> Dictionary:
	if not EVENTS.has(event_id):
		push_error("ไม่พบเหตุการณ์ '%s' ใน ChatDatabase.EVENTS (ที่มี: %s)" \
			% [event_id, ", ".join(PackedStringArray(EVENTS.keys()))])
		return {}
