...
## ==============================================
## บรรทัดตั้งธงเนื้อเรื่อง — `{"set": "ชื่อธงใน GameState"}`
## ==============================================
## ใช้บอกว่า "ถึงตรงนี้ในบท เรื่องนี้เกิดขึ้นแล้ว" เช่นโชยื่นขนมปังให้กลางบท
## → `{"set": "has_bread"}` แล้วช่องของกับไอคอนเควสอัปเดตตามเองทันที
##
## 🚨 **เขียนไว้ในบท ไม่ใช่ให้ NPC ตั้งธงตอนบทจบ** — จังหวะที่ของเปลี่ยนมือ
## เป็นเรื่องของ**บท** คนเขียนบทควรย้ายจุดนั้นได้เองโดยไม่ต้องแตะสคริปต์ NPC
## · และบทที่จบด้วย `"hold"` จะปิดตัวเองก่อนถึงบรรทัดสุดท้ายเสมอ ถ้าไปผูกกับ
##   "ตอนบทจบ" จะได้ธงที่ตั้งบ้างไม่ตั้งบ้างขึ้นกับว่าบทนั้นจบยังไง
##
## ⚠️ **ตั้งได้เฉพาะเป็น `true`** — บทเล่าเรื่องที่เดินไปข้างหน้า ไม่มีเคสย้อนธงกลับ
## อยากปิดธงค่อยเพิ่ม `{"unset": ...}` ตอนมีที่ใช้จริง
##
## ⚠️ พิมพ์ชื่อธงผิด → `push_error` ไม่ใช่เงียบ ๆ ข้ามไป ไม่งั้นเนื้อเรื่องจะค้าง
## อยู่ขั้นเดิมตลอดกาลโดยไม่มีอะไรบอก (กฎเดียวกับ `"if"` / `"unless"` ของตัวเลือก)
func _run_set_flag_line(line: Dictionary) -> void:
	var flag: String = str(line.get("set", "")).strip_edges()
	if flag.is_empty():
		push_error("dialogue_system: บรรทัด `set` ไม่ได้ระบุชื่อธง")
	elif not flag in GameState:
		push_error("dialogue_system: ไม่มีตัวแปรชื่อ '%s' ใน GameState (บรรทัด `set`)" % flag)
	else:
		GameState.set(flag, true)

	## ไปบรรทัดถัดไปทันที — บรรทัดนี้ไม่มีอะไรให้ผู้เล่นอ่านหรือกด
	##
	## 🚨 **ต้องเช็คว่ายังมีบรรทัดถัดไปไหม** — `{"set": ...}` ที่เป็นบรรทัดสุดท้าย
	## แล้วเรียก `show_line()` ทื่อ ๆ จะอ่านเลย index สุดท้ายจน crash
	index += 1
...
func show_line():
	...
	## บรรทัดตั้งธงเนื้อเรื่อง — ไม่มีอะไรให้แสดง ทำแล้วไปบรรทัดถัดไปทันที
	if line.has("set"):
		_run_set_flag_line(line)
		return
